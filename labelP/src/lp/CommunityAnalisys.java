 ...
 *
 */
public class CommunityAnalisys {
	...
	public void readIt(int numCommunities, String fileMembership, String fileLanguages) throws IOException {
		...
		communityList = new ArrayList<>(numCommunities);
		for (int i=0; i<numCommunities; i++) {
			communityList.add(new Community());
		}
				
		
                BufferedReader brLanguages;
            try (BufferedReader brMembership = new BufferedReader(new FileReader(fileMembership))) {
                brLanguages = new BufferedReader(new FileReader(fileLanguages));
	...
	public void calcStats() {
		...
		HashMap<String,Integer> langCounts = new HashMap<>();
		for (int i=0; i<communityList.size(); i++) {
			...
			for (int j=0; j<comm.getSize(); j++) {
				...
				if (count==null) {
                                    Integer put;
                                    put = langCounts.put(lang, 1);
				} else {
                                    Integer put;
                                    put = langCounts.put(lang, count+1);
				}
	...
	public void writeCommunities(String file) throws IOException {
            try (FileOutputStream fso = new FileOutputStream(file); OutputStreamWriter fileWriter = new OutputStreamWriter(fso,Charset.forName("UTF-8"))) {
                Community c;
	...
	public static void main(String[] args) throws IOException {
		CommunityAnalisys ca = new CommunityAnalisys();
	...
	private class Community {
		...
		public ArrayList<LanguageNode> nodeList = new ArrayList<>();
